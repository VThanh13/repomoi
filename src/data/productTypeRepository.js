...
class ProductTypeRepository extends BaseRepository {
    ...
    async findOne(key, value) {
        const coll = await ProductTypeDto.findOne({
            [key]: value
        });
        return ProductTypeModel.fromMongo(coll);

    }
    async findData(data) {
        const docs = await ProductTypeDto.find(data);
        return docs.map((item) => ProductTypeModel.fromMongo(item));
    }
    ...
    async deleteMany(key, value) {
        ...
        const coll = await ProductTypeDto.delete({
            [key]: { $in: value }
        });
        return coll;
    }
