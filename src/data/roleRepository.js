...
class RoleRepository extends BaseRepository {
    ...
    async createOne(data) {
        ...
        return RoleModel.fromMongo(doc);
    }
    async findOne(key, value) {
        const coll = await RoleDto.findOne({
            [key]: value });
        return RoleModel.fromMongo(coll);
    }
    async findData(data) {
        const docs = await RoleDto.find(data);
        return docs.map((item) => RoleModel.fromMongo(item));
    }
    ...
    async deleteMany(key, value) {
        ...
        const coll = await RoleDto.delete({
            [key]: { $in: value } });
        return coll;
    }
    async search(data) {
